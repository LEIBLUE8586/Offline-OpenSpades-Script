...
CONFIG_FILE = os.path.join(SCRIPT_DIR, "launcher_config.json")  # New config file
...
class OpenSpadesLauncher:
    ...
    def setup_ui(self):
        ...
        self.path_entry.insert(0, self.config["openspades_path"])  # Initialize with loaded config
        ...
        self.ip_entry.insert(0, self.config["server_ip"])  # Initialize with loaded config
        self.ip_entry.grid(row=0, column=1, padx=5)
        
        tk.Label(addr_frame, text="Port:").grid(row=0, column=2, padx=5)
        self.port_entry = tk.Entry(addr_frame, width=8)
        self.port_entry.insert(0, self.config["server_port"])  # Initialize with loaded config
